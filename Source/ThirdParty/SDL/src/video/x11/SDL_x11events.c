...
X11_ReconcileKeyboardState(_THIS)
{
    ...
    if (X11_XQueryPointer(display, DefaultRootWindow(display), &junk_window, &junk_window, &x, &y, &x, &y, &mask)) {
        ...
        if (!!(mask & num_mask) != !!(SDL_GetModState() & KMOD_NUM)) {
            SDL_SendKeyboardKey(SDL_PRESSED, (Uint32)(SDL_SCANCODE_NUMLOCKCLEAR), SDL_SCANCODE_NUMLOCKCLEAR);
            if (numlockState == SDL_RELEASED) {
                SDL_SendKeyboardKey(SDL_RELEASED, (Uint32)(SDL_SCANCODE_NUMLOCKCLEAR), SDL_SCANCODE_NUMLOCKCLEAR);
            }
        }
    }
