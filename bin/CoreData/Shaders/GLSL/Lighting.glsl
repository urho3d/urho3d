...
float GetAtten(vec3 normal, vec3 worldPos, out vec3 lightDir)
{
        ...
        float d = length(cLightPosPS.xyz - worldPos);
        float r = clamp(1.0 - d * cLightPosPS.w, 0.0, 1.0);

        float i = 1.0 / (lightDist * lightDist);
        return clamp(dot(normal, lightDir), 0.0, 1.0) * i * r;
    #endif
}
