...
// Urho3D - remove hardcoded defines and replace 'GCC' custom define with more standard '__GNUC__'
...
{
#ifdef ENABLE_CPP_VERSION // I am not yet convinced that the NEON version is faster than the C version of this
...
{
#ifdef ENABLE_CPP_VERSION // I am not convinced that the NEON version is faster than the C version yet.
    __m128 ret;
    ret[0] = a[i & 0x3];
    ret[1] = a[(i >> 2) & 0x3];
    ret[2] = b[(i >> 4) & 0x03];
...
FORCE_INLINE __m128 _mm_shuffle_ps_function(__m128 a, __m128 b)
{
    switch (i)
    {
        case _MM_SHUFFLE(1, 0, 3, 2): return _mm_shuffle_ps_1032(a, b);
        case _MM_SHUFFLE(2, 3, 0, 1): return _mm_shuffle_ps_2301(a, b);
        case _MM_SHUFFLE(3, 2, 1, 0): return _mm_shuffle_ps_3210(a, b);
        case _MM_SHUFFLE(0, 0, 1, 1): return _mm_shuffle_ps_0011(a, b);
        case _MM_SHUFFLE(0, 0, 2, 2): return _mm_shuffle_ps_0022(a, b);
        case _MM_SHUFFLE(2, 2, 0, 0): return _mm_shuffle_ps_2200(a, b);
        case _MM_SHUFFLE(3, 2, 0, 2): return _mm_shuffle_ps_3202(a, b);
        case _MM_SHUFFLE(1, 1, 3, 3): return _mm_shuffle_ps_1133(a, b);
        case _MM_SHUFFLE(2, 0, 1, 0): return _mm_shuffle_ps_2010(a, b);
        case _MM_SHUFFLE(2, 0, 0, 1): return _mm_shuffle_ps_2001(a, b);
        case _MM_SHUFFLE(2, 0, 3, 2): return _mm_shuffle_ps_2032(a, b);
...
// Urho3D - add more mapping

// Divide packed single-precision (32-bit) floating-point elements in a by packed elements in b, and store the results in dst.
FORCE_INLINE __m128 _mm_div_ps(__m128 a, __m128 b)
{
    float32x4_t v, m;
    v = vrecpeq_f32(b);         // v ~ 1/b
    m = vrecpsq_f32(b, v);      // m = (2-v*b)
    v = vmulq_f32(v, m);        // vv = v*m ~~ 1/b
    m = vrecpsq_f32(b, v);      // mm = (2-vv*b)
    v = vmulq_f32(v, a);        // a*vv
    return vmulq_f32(v, m);     // (a*vv)*(2-vv*b) = a*(vv(2-vv*b)) ~~~ a/b
}
...
{
    float32x4_t v, v2, m;
    v = vrsqrteq_f32(a);        // v ~ 1/√a
    v2 = vmulq_f32(v, v);       // v2 = v*v
    m = vrsqrtsq_f32(a, v2);    // m = (3-v2*a)/2
    v = vmulq_f32(v, m);        // vv = v*m ~~ 1/√a
    v2 = vmulq_f32(v, v);       // vv2 = vv*vv
    m = vrsqrtsq_f32(a, v2);    // mm = (3-vv2*a)/2
    return vmulq_f32(v, m);     // vvv = vv*mm ~~~ 1/√a
}

// Copy single-precision (32-bit) floating-point element a to the lower element of dst, and zero the upper 3 elements.
FORCE_INLINE __m128 _mm_set_ss(float a)
{
    return vsetq_lane_f32(a, vdupq_n_f32(0), 0);
}
...
// Add the lower single-precision (32-bit) floating-point element in a and b, store the result in the lower element of dst,
// and copy the upper 3 packed elements from a to the upper elements of dst.
FORCE_INLINE __m128 _mm_add_ss(__m128 a, __m128 b)
{
	return _mm_move_ss(a, vaddq_f32(a, b));
}
...
{
	return _mm_move_ss(a, vsubq_f32(a, b));
}

// Multiply the lower single-precision (32-bit) floating-point element in a and b, store the result in the lower element of dst,
// and copy the upper 3 packed elements from a to the upper elements of dst.
FORCE_INLINE __m128 _mm_mul_ss(__m128 a, __m128 b)
{
	return _mm_move_ss(a, vmulq_f32(a, b));
}

// Divide the lower single-precision (32-bit) floating-point element in a by the lower single-precision (32-bit) floating-point element in b,
// store the result in the lower element of dst, and copy the upper 3 packed elements from a to the upper elements of dst.
...
FORCE_INLINE __m128 _mm_loadl_pi(__m128 a, const __m64 * mem_addr)
{
    return vcombine_f32(vld1_f32(reinterpret_cast<const float32_t*>(mem_addr)), vget_high_f32(a));
}
...
{
    vgetq_lane_f32(a, 0) >= vgetq_lane_f32(b, 0) ? 1 : 0;
}

// Macro: Transpose the 4x4 matrix formed by the 4 rows of single-precision (32-bit) floating-point elements in
// row0, row1, row2, and row3, and store the transposed matrix in these vectors (row0 now contains column 0, etc.).
