...
void PositionGizmo()
{
    ...
    if (editMode != lastGizmoMode)
    {
        ...
        case EDIT_ROTATE:
            gizmo.model = cache.GetResource("Model", "Editor/Models/RotateAxes.mdl");
            break;

        case EDIT_SCALE:
            gizmo.model = cache.GetResource("Model", "Editor/Models/ScaleAxes.mdl");
            break;
        }
...
void UseGizmo()
{
    ...
    {
        gizmo.materials[0] = cache.GetResource("Material", gizmoAxisX.selected ? "Editor/Materials/BrightRedUnlit.xml" :
            "Editor/Materials/RedUnlit.xml");
        gizmoAxisX.lastSelected = gizmoAxisX.selected;
    }
    if (gizmoAxisY.selected != gizmoAxisY.lastSelected)
    {
        gizmo.materials[1] = cache.GetResource("Material", gizmoAxisY.selected ? "Editor/Materials/BrightGreenUnlit.xml" :
            "Editor/Materials/GreenUnlit.xml");
        gizmoAxisY.lastSelected = gizmoAxisY.selected;
    }
    if (gizmoAxisZ.selected != gizmoAxisZ.lastSelected)
    {
        gizmo.materials[2] = cache.GetResource("Material", gizmoAxisZ.selected ? "Editor/Materials/BrightBlueUnlit.xml" :
            "Editor/Materials/BlueUnlit.xml");
