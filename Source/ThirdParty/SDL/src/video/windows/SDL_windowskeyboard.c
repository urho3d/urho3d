...
// Modified by Aster Jian and Alex Fuller for Urho3D
...
WIN_InitKeyboard(_THIS)
{
    ...
    if (GetKeyState(VK_CAPITAL) & 0x0001) {
        SDL_SendKeyboardKey(SDL_PRESSED, 0, SDL_SCANCODE_CAPSLOCK);
        SDL_SendKeyboardKey(SDL_RELEASED, 0, SDL_SCANCODE_CAPSLOCK);
    }
    if (GetKeyState(VK_NUMLOCK) & 0x0001) {
        SDL_SendKeyboardKey(SDL_PRESSED, 0, SDL_SCANCODE_NUMLOCKCLEAR);
        SDL_SendKeyboardKey(SDL_RELEASED, 0, SDL_SCANCODE_NUMLOCKCLEAR);
    }
    if (GetKeyState(VK_SCROLL) & 0x0001) {
        SDL_SendKeyboardKey(SDL_PRESSED, 0, SDL_SCANCODE_SCROLLLOCK);
        SDL_SendKeyboardKey(SDL_RELEASED, 0, SDL_SCANCODE_SCROLLLOCK);
    }
}
