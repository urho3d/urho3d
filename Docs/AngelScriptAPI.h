...
void ApplyAttributes();
void BringToFront();
UIElement CreateChild(const String&, const String& = String ( ), uint = M_LIMITS<unsigned>::Max);
...
void SetParent(UIElement, uint = M_LIMITS<unsigned>::Max);
...
{
// Methods:
void AddChild(Node, uint = M_LIMITS<unsigned>::Max);
void AddTag(const String&);
void AddTags(const String&, int8 = ';');
void ApplyAttributes();
...
bool LoadXML(const XMLElement&, bool = false);
void MarkNetworkUpdate() const;
Array<RayQueryResult> Raycast(const Ray&, RayQueryLevel = RAY_TRIANGLE, float = M_LIMITS<float>::infinity(), uint8 = DRAWABLE_ANY, uint = DEFAULT_VIEWMASK) const;
RayQueryResult RaycastSingle(const Ray&, RayQueryLevel = RAY_TRIANGLE, float = M_LIMITS<float>::infinity(), uint8 = DRAWABLE_ANY, uint = DEFAULT_VIEWMASK) const;
void Remove();
void RemoveAttributeAnimation(const String&);
void RemoveInstanceDefault();
...
bool AddPackageFile(PackageFile, uint = M_LIMITS<unsigned>::Max);
bool AddPackageFile(const String&, uint = M_LIMITS<unsigned>::Max);
bool AddResourceDir(const String&, uint = M_LIMITS<unsigned>::Max);
...
{
// Methods:
void AddControlPoint(Node, uint = M_LIMITS<unsigned>::Max);
void ApplyAttributes();
...
void SetParent(UIElement, uint = M_LIMITS<unsigned>::Max);
void SetPosition(int, int);
void SetSelection(uint, uint = M_LIMITS<unsigned>::Max);
void SetSize(int, int);
bool SetStyle(const String&, XMLFile = null);
bool SetStyle(const XMLElement&);
bool SetStyleAuto(XMLFile = null);
...
float M_DEGTORAD;
float M_DEGTORAD_2;
float M_LIMITS<float>::epsilon();
float M_HALF_PI;
float M_LIMITS<float>::infinity();
float M_LARGE_EPSILON;
float M_LARGE_VALUE;
int M_LIMITS<signed>::Max;
uint M_LIMITS<unsigned>::Max;
int M_LIMITS<signed>::Min;
uint M_MIN_UNSIGNED;
float M_PI;
float M_RADTODEG;
float PIXEL_SIZE;
