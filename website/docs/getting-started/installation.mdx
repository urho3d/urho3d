...
You need an installed Urho3D library for the desired target platform to proceed. If you have been following along the previous section to install the library then you should be able to create a new project using the library in no time. The most commonly encountered issue by newbie in the past is figuring out how to tell the build script where to find the **correct** Urho3D library to use. The new rake tasks have been designed to address this issue by providing opinionated sane default for most of the use cases, but without sacrificing the possibility to override the default when necessary. All you need is to execute the following command to create a new UrhoApp project.
...
### Using CLI {#build-urhoapp-using-cli}
...
```bash
# When compiling natively on the RPI board
RPI_ABI=RPI3 PLATFORM=rpi rake
...
For Android platform, you would have to use other gradle task (e.g., `./gradlew installDebug`) to deploy the UrhoApp to an Android Emulator or actual device.
