...
void UpdateView(float timeStep)
{
    ...
    if (ui.focusElement is null)// && !input.keyDown[KEY_LCTRL])
    {
        ...
        if (input.mouseMoveWheel != 0 && ui.GetElementAt(ui.cursor.position) is null)
        {
            ...
            {
                cameraNode.Translate(Vector3(0, 0, -cameraBaseSpeed) * -input.mouseMoveWheel*5 * timeStep *
                    speedMultiplier);
            }
            else
            {
    ...
    {
		if (mouseMiddleBtnPan && input.keyDown[KEY_LSHIFT])
			CamView(true);
		else
			CamView();
    }
