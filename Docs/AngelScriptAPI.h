...
void ApplyAttributes();
void BringToFront();
UIElement CreateChild(const String&, const String& = String ( ), uint = std::numeric_limits<unsigned>::max());
...
void SetParent(UIElement, uint = std::numeric_limits<unsigned>::max());
...
{
// Methods:
void AddChild(Node, uint = std::numeric_limits<unsigned>::max());
void AddTag(const String&);
void AddTags(const String&, int8 = ';');
void ApplyAttributes();
...
bool LoadXML(const XMLElement&, bool = false);
void MarkNetworkUpdate() const;
Array<RayQueryResult> Raycast(const Ray&, RayQueryLevel = RAY_TRIANGLE, float = std::numeric_limits<float>::infinity(), uint8 = DRAWABLE_ANY, uint = DEFAULT_VIEWMASK) const;
RayQueryResult RaycastSingle(const Ray&, RayQueryLevel = RAY_TRIANGLE, float = std::numeric_limits<float>::infinity(), uint8 = DRAWABLE_ANY, uint = DEFAULT_VIEWMASK) const;
void Remove();
void RemoveAttributeAnimation(const String&);
void RemoveInstanceDefault();
...
bool AddPackageFile(PackageFile, uint = std::numeric_limits<unsigned>::max());
bool AddPackageFile(const String&, uint = std::numeric_limits<unsigned>::max());
bool AddResourceDir(const String&, uint = std::numeric_limits<unsigned>::max());
...
{
// Methods:
void AddControlPoint(Node, uint = std::numeric_limits<unsigned>::max());
void ApplyAttributes();
...
void SetParent(UIElement, uint = std::numeric_limits<unsigned>::max());
void SetPosition(int, int);
void SetSelection(uint, uint = std::numeric_limits<unsigned>::max());
void SetSize(int, int);
bool SetStyle(const String&, XMLFile = null);
bool SetStyle(const XMLElement&);
bool SetStyleAuto(XMLFile = null);
...
float M_LARGE_VALUE;
int std::numeric_limits<signed>::max();
uint std::numeric_limits<unsigned>::max();
int std::numeric_limits<signed>::min();
uint M_MIN_UNSIGNED;
float M_PI;
float M_RADTODEG;
float PIXEL_SIZE;
